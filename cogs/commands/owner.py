...
class Owner (commands .Cog ):
    ...
    async def _slist (self ,ctx ):
        ...
        entries =entries ,
        description ="",
        title =f"Guild List of JagBot-bot [{len(self.client.guilds)}]",
        color =0x000000 ,
        per_page =10 ),
        ctx =ctx )
    ...
    async def _restart (self ,ctx :Context ):
        await ctx .reply ("Rebooting JagBot...")
        restart_program ()
    ...
    async def own_list (self ,ctx ):
        ...
        paginator =Paginator (source =DescriptionEmbedPaginator (
        entries =entries ,
        title =f"JagBot Owners [{len(nplist)}]",
        description ="",
        per_page =10 ,
        color =0x000000 ),
        ctx =ctx )
...
class Badges (commands .Cog ):
    ...
    async def badges (self ,ctx ,member :discord .Member =None ):
        ...
        processing_message =await ctx .send ("<a:JagBot_loading:1372527554761855038> Loading your profile...")
